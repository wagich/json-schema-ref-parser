...
describe("Schema with external $refs", () => {
  it("should parse successfully from an absolute path", async () => {
    ...
    expect(parser.$refs.paths()).to.deep.equal([path.abs("specs/external/external.json")]);
  });

  it("should parse successfully from a relative path", async () => {
    let parser = new $RefParser();
    const schema = await parser.parse(path.rel("specs/external/external.json"));
    expect(schema).to.equal(parser.schema);
    expect(schema).to.deep.equal(parsedSchema.schema);
    expect(parser.$refs.paths()).to.deep.equal([path.abs("specs/external/external.json")]);
  });
  ...
  it("should parse successfully from a url", async () => {
    let parser = new $RefParser();
    const schema = await parser.parse(path.url("specs/external/external.json"));
    expect(schema).to.equal(parser.schema);
    expect(schema).to.deep.equal(parsedSchema.schema);
    expect(parser.$refs.paths()).to.deep.equal([path.url("specs/external/external.json")]);
  });

  it("should resolve successfully from an absolute path", helper.testResolve(
    path.abs("specs/external/external.json"),
    path.abs("specs/external/external.json"), parsedSchema.schema,
    path.abs("specs/external/definitions/definitions.json"), parsedSchema.definitions,
    path.abs("specs/external/definitions/name.json"), parsedSchema.name,
    path.abs("specs/external/definitions/required-string.json"), parsedSchema.requiredString
  ));

  it("should resolve successfully from a relative path", helper.testResolve(
    path.rel("specs/external/external.json"),
    path.abs("specs/external/external.json"), parsedSchema.schema,
    path.abs("specs/external/definitions/definitions.json"), parsedSchema.definitions,
    path.abs("specs/external/definitions/name.json"), parsedSchema.name,
    path.abs("specs/external/definitions/required-string.json"), parsedSchema.requiredString
  ...
  it("should resolve successfully from a url", helper.testResolve(
    ...
    path.url("specs/external/external.json"), parsedSchema.schema,
    path.url("specs/external/definitions/definitions.json"), parsedSchema.definitions,
    path.url("specs/external/definitions/name.json"), parsedSchema.name,
    path.url("specs/external/definitions/required-string.json"), parsedSchema.requiredString
  ));

  it("should dereference successfully", async () => {
    let parser = new $RefParser();
    const schema = await parser.dereference(path.rel("specs/external/external.json"));
    expect(schema).to.equal(parser.schema);
  ...
  it("should bundle successfully", async () => {
    let parser = new $RefParser();
    const schema = await parser.bundle(path.rel("specs/external/external.json"));
    expect(schema).to.equal(parser.schema);
